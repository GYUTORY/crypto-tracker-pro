...
이 프로젝트는 **Clean Architecture** 원칙을 적용한 계층화된 구조를 사용했습니다. 각 계층의 역할을 명확히 나누고 의존성 방향을 일관되게 유지해서 코드를 관리하기 쉽게 만들었습니다.
...
- **위치**: `src/presentation/`
- **역할**: HTTP API 엔드포인트 제공, 요청/응답 처리
- **구성요소**: Controllers, DTOs, Swagger 문서
- **특징**: 비즈니스 로직 없음, Use Cases 호출만 담당

#### 2. Application Layer (애플리케이션 계층)
- **위치**: `src/application/`
- **역할**: 비즈니스 로직 조합 및 워크플로우 정의
- **구성요소**: Use Cases, Services, Workflows
- **특징**: 도메인 객체와 인프라스트럭처 간 중재

#### 3. Domain Layer (도메인 계층)
- **위치**: `src/domain/`
- **역할**: 핵심 비즈니스 규칙과 엔티티 정의
- **구성요소**: Entities, Repository Interfaces, Schemas
- **특징**: 외부 시스템에 전혀 의존하지 않는 순수한 도메인

#### 4. Infrastructure Layer (인프라스트럭처 계층)
- **위치**: `src/infrastructure/`
- **역할**: 외부 시스템과의 실제 통신 구현
- **구성요소**: Repository 구현체들, External Services
- **특징**: Domain Layer의 인터페이스 구현
