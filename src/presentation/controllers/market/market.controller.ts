...
export class MarketController extends BaseService {
  ...
  @ApiOperation({
    summary: '시장 요약 정보 조회',
    description: '전체 암호화폐 시장의 요약 정보를 조회합니다.'
  })
  @ApiResponse({
    status: 200,
    description: '시장 요약 정보 조회 성공',
    type: BaseResponseDto<MarketStatsResponseDto>
  })
  @ApiResponse({
    status: 500,
  ...
  async getMarketSummary(): Promise<BaseResponseDto<MarketStatsResponseDto>> {
    const result = await this.getMarketStatsUseCase.execute();

    return this.success(result, '시장 요약 정보 조회 완료');
  }
}
